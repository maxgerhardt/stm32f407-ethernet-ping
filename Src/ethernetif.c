...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  for(q = p; q != NULL; q = q->next)
    {
	  //hexDump("low_level_output", p->payload, p->len);
  ...
  //hexDump("Ethernet buffer", (uint8_t *)(heth.TxDesc->Buffer1Addr), framelength);
...
__weak void ethernetif_notify_conn_changed(struct netif *netif)
{
  ...
  */
	printf("ethernetif_notify_conn_changed() called: %p\n", netif);
}
/* USER CODE END 8 */ 
